...
def fetch_data(tag: str):
    ...
    try:
        ...
        # Hata durumlarını detaylı yakala
        if response.status_code != 200:
            return {"html": f"<div class='p-3 bg-red-900/30 border border-red-500 rounded'>❌ Hata ({response.status_code}): {response.reason}</div>"}
        ...
        return {"html": f"<div class='p-3 bg-orange-900/30 border border-orange-500 rounded'>🚨 Sistem Hatası: {str(e)}</div>"}from fastapi import FastAPI, HTTPException
from fastapi.responses import HTMLResponse
import requests
